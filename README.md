...
- The production application is available at the verified URL above. The exact release completed the browser E2E journey and bounded safety sweep; measured page HTTP RPC evidence, lifecycle/readback evidence, and wallet-session regressions are recorded in the verification documents. Provider-extension physical request counts are not observable in the supported browser surface and are not inferred.
